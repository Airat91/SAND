...
def regs_handler(Proj):
    ...
    for struct_name in Proj.struct_list:
        ...
        for reg_name in struct["reg_list"]:
            ...
            if name not in reg_name_list:
                ...
                reg_name_list.append(name)
            #1.2 Check reg type
            type = reg.prop_list["sand_prop_base_t"]["type"]["value"]
            if type == None:
                ...
                if var_type not in sand_reg.sand_var_t:
                    ...
                    reg.prop_list["sand_prop_base_t"]["type"]["value"] = var_type

            if reg not in regs_to_delete:
                reg.prop_list["sand_prop_base_t"]["ind"]["value"] = reg_index
                reg_index += 1
    ...
    for reg_name in Proj.prop_list["sand_prop_mdb_t"]["reg_list"]:
        reg = Proj.prop_list["sand_prop_mdb_t"]["reg_list"][reg_name]
        struct_name = get_struct_of_reg(reg_name, Proj)
        reg_byte_len = int(reg.prop_list["sand_prop_base_t"]["array_len"]["value"]) * \
                       sand_reg.sand_var_t[reg.prop_list["sand_prop_base_t"]["type"]["value"]]["byte_num"]
        # mdb_addr reading
    ...
    for reg_name in Proj.prop_list["sand_prop_mdb_t"]["reg_list"]:
        ...
        if isinstance(mdb_addr, str):
            if mdb_addr != "auto":
                ...
                space_found = False
                mdb_addr_list.sort()
                mdb_addr_head = sand_reg.SAND_LIMITS["mdb_addr_min"]
                ...
                if space_found == False:
                    if sand_reg.SAND_LIMITS["mdb_addr_max"] - mdb_addr_head >= math.ceil(reg_byte_len / 2):
                        space_found = True
                    else:
                ...
                if space_found == True:
                    ...
                    reg.prop_list["sand_prop_mdb_t"]["mdb_addr"]["value"] = mdb_addr_head
                    for i in range(0, math.ceil(reg_byte_len / 2)):
                        mdb_addr_list.append(mdb_addr_head)
                        mdb_addr_head += 1
                    mdb_addr_list.sort()
    #2.2.3 Sort prop_list in Proj by MDB address
    prop_list = deepcopy(Proj.prop_list["sand_prop_mdb_t"]["reg_list"])
    Proj.prop_list["sand_prop_mdb_t"]["reg_list"] = {}
    for mdb_ind in mdb_addr_list:
        for reg_name in prop_list:
            reg = Proj.prop_list["sand_prop_base_t"]["reg_list"][reg_name]
            if reg.prop_list["sand_prop_mdb_t"]["mdb_addr"]["value"] == mdb_ind:
    ...
    for reg_name in Proj.prop_list["sand_prop_save_t"]["reg_list"]:
        ...
        reg_byte_len = int(reg.prop_list["sand_prop_base_t"]["array_len"]["value"]) * \
                       sand_reg.sand_var_t[reg.prop_list["sand_prop_base_t"]["type"]["value"]]["byte_num"]
        # save_addr reading
        save_addr = reg.prop_list["sand_prop_save_t"]["save_addr"]["value"]
    ...
    for reg_name in Proj.prop_list["sand_prop_save_t"]["reg_list"]:
        ...
        if isinstance(save_addr, str):
            if save_addr != "auto":
                ...
                space_found = False
                save_addr_list.sort()
                save_addr_head = sand_reg.SAND_LIMITS["save_addr_min"]
                if len(save_addr_list) == 0:
                    ...
                    for ind in range(len(save_addr_list)):
                        if (save_addr_list[ind] - save_addr_head) >= reg_byte_len:
                            ...
                            ind += 1
                if space_found == False:
                    if sand_reg.SAND_LIMITS["save_addr_max"] - save_addr_head >= reg_byte_len:
                        space_found = True
                    else:
                        Proj.errors["err_msg"].append(
                ...
                if space_found == True:
                    # If empty range is found
                    reg.prop_list["sand_prop_save_t"]["save_addr"]["value"] = save_addr_head
                    for i in range(0, reg_byte_len):
    ...
    for save_ind in save_addr_list:
        for reg_name in prop_list:
            reg = Proj.prop_list["sand_prop_base_t"]["reg_list"][reg_name]
            if reg.prop_list["sand_prop_save_t"]["save_addr"]["value"] == save_ind:
    ...
    for reg_name in Proj.prop_list["sand_prop_access_t"]["reg_list"]:
        ...
        struct_name = get_struct_of_reg(reg_name, Proj)
        access_lvl = reg.prop_list["sand_prop_access_t"]["access_lvl"]["value"]
